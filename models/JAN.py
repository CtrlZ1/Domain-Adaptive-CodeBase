...
from discrepancies.MMD import MultipleKernelMaximumMeanDiscrepancy, GaussianKernel, \
    JointMultipleKernelMaximumMeanDiscrepancy, Theta
from utils import model_feature_tSNE

def model_test(model,targetData,backbone_name):
    model.eval()
...
def test_process(model,sourceTestDataLoader,taragetTestDataLoader, device, args):
    ...
    with torch.no_grad():
        for data, sourceLabel in sourceTestDataLoader:
            ...
            pre_label = model_test(model,data,args.backbone_name)
...
class JANModel(nn.Module):
    def __init__(self,device,args):
        ...
        self.backbone=network_dict[args.backbone_name]()
        self.device=device
        self.args=args
    ...
    def forward(self,sourceData, targetData,jmmd):
        if self.args.backbone_name=='AlexNetFc_for_layerWiseAdaptation':
